 ...
 */
function withCycleGuard(seen, node, depth, path, fn) {
 ...
 */
function assertDepthOk(depth, path) {
    if (depth > MAX_VALIDATION_DEPTH) {
        throw new PretextPdfError('VALIDATION_ERROR', `${path}: nesting depth exceeds ${MAX_VALIDATION_DEPTH}`);
    }
}
...
export function validate(doc, options) {
    ...
    if (doc.encryption) {
        ...
        if (doc.encryption.userPassword === undefined) {
            console.warn('[pretext-pdf] doc.encryption is set without userPassword — ' +
...
function validateElement(el, index, loadedFamilies, strict, errors, depth = 0, seen = new WeakSet(), options) {

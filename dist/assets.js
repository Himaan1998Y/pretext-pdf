 ...
 */
async function resolveSvgContent(el, allowedFileDirs) {
    if (el.svg)
...
// ─── QR Code generator ────────────────────────────────────────────────────────
async function generateQrSvg(el) {
    let qrLib;
    try {
...
// ─── Barcode generator ────────────────────────────────────────────────────────
async function generateBarcodeSvg(el) {
    let bwip;
    try {
...
async function generateChartSvg(el, contentWidth) {
    let vegaLite;
    let vega;
    try {
 ...
 */
async function allSettledWithLimit(items, limit) {
 ...
 */
async function loadVectorAssets(doc, pdfDoc, imageMap, contentWidth, allowedDirs, warn) {
    const tasks = [];
...
async function loadImageBytes(el, key, allowedDirs) {

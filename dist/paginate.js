 ...
 */
function assertValidCalloutBlock(block) {
    const cd = block.calloutData;
    if (!cd ||
        !isFiniteNumber(cd.titleHeight) ||
        !isFiniteNumber(cd.paddingV) ||
        !isFiniteNumber(cd.paddingH)) {
        throw new PretextPdfError('PAGINATION_FAILED', 'MeasuredBlock contract violated: callout block has missing or non-finite calloutData fields (titleHeight, paddingV, paddingH)');
    }
}
/**
 ...
 */
function assertValidBlockquoteBlock(block) {
    if (!isFiniteNumber(block.blockquotePaddingV) ||
        !isFiniteNumber(block.blockquotePaddingH) ||
        !isFiniteNumber(block.blockquoteBorderWidth)) {
        throw new PretextPdfError('PAGINATION_FAILED', 'MeasuredBlock contract violated: blockquote block has missing or non-finite padding/border fields');
    }
}
/**
 ...
 */
function validateMeasuredBlocks(blocks) {
 ...
 */
function calloutTitleHeight(block, isFirstChunk) {
    if (block.element.type !== 'callout')
        return 0;
    const cd = block.calloutData;
 ...
 */
function verticalPadding(block, fallback) {
    if (block.element.type === 'callout') {
        return block.calloutData.paddingV;
    }
    if (block.element.type === 'blockquote') {
        return block.blockquotePaddingV;
    }
    return fallback;
}
/**
 ...
 *   cannot fit.
 */
export function paginate(blocks, pageContentHeight, config = { minOrphanLines: 2, minWidowLines: 2 }) {
 ...
 */
function paginateTable(block, pages, currentY, pageContentHeight) {
 ...
 */
function placeBlock(block, pages, currentY, pageContentHeight, config) {
 ...
 */
function splitBlock(block, pages, currentY, pageContentHeight, config) {
...
function pushNewPage(pages) {
    pages.push(newPage(pages.length));
}
function addToCurrentPage(pages, block) {
 ...
 */
function countRichLinesInHeight(richLines, startIdx, available) {
    let count = 0;
    let used = 0;
    for (let i = startIdx; i < richLines.length; i++) {
        const h = richLines[i].lineHeight;
        if (used + h <= available + EPSILON) {
            count++;
            used += h;
        }
        else

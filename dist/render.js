...
export async function renderDocument(paginatedDoc, doc, fontMap, imageMap, pdfDoc, geo, plugins, logger) {
    ...
    const tokenExtra = { date: dateStr };
    if (doc.metadata?.author)
        tokenExtra.author = doc.metadata.author;
    for (const renderedPage of paginatedDoc.pages) {
        ...
        if (renderedPage.footnoteItems && renderedPage.footnoteItems.length > 0) {
            try {
                renderFootnoteZone(pdfPage, renderedPage.footnoteItems, renderedPage.footnoteZoneHeight ?? 0, fontMap, doc, geo);
            }
            catch (e) {
...
// ─── Block routing ────────────────────────────────────────────────────────────
function renderBlock(pdfPage, pagedBlock, geo, fontMap, imageMap, pdfDoc, footnoteNumbering, plugins) {
    ...
    switch (element.type) {
        ...
        default: {
            ...
            throw new PretextPdfError('RENDER_FAILED', `renderBlock: unhandled element type '${String(unknownType)}'. This is a bug — validate() should have rejected it.`);
        }
    }
}

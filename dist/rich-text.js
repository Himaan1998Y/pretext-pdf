...
const _sentinelWidthCache = new Map();
 ...
 */
async function measureTokenWidth(text, fontSize, fontFamily, fontWeight, fontStyle) {
    if (!text)
        return 0;
 ...
 */
export async function measureRichText(spans, fontSize, lineHeight, contentWidth, align, doc) {
    ...
    for (const span of spans) {
        ...
        if (smallCaps)
            spanEffectiveFontSize = spanEffectiveFontSize * 0.8;
    ...
    const richLines = composedLines.map(({ fragments, totalWidth, lineHeight: composedLineHeight }) => {
        let offset = 0;
        if (align === 'center')
            offset = Math.max(0, (contentWidth - totalWidth) / 2);
        else if (align === 'right')
            offset = Math.max(0, contentWidth - totalWidth);
        // Recompute fragment x positions from left
        let x = offset;
        const positionedFragments = fragments.map(f => {
            const fragment = { text: f.text, fontKey: f.fontKey, fontSize: f.fontSize, color: f.color, x, width: f.width, underline: f.underline ?? false, strikethrough: f.strikethrough ?? false, ...(f.url !== undefined ? { url: f.url } : {}), ...(f.yOffset !== undefined ? { yOffset: f.yOffset } : {}), ...(f.footnoteRef !== undefined ? { footnoteRef: f.footnoteRef } : {}), ...(f.letterSpacing !== undefined && f.letterSpacing > 0 ? { letterSpacing: f.letterSpacing } : {}) };
            x += f.width;
            return fragment;
        });

 ...
 */
export function buildOutlineTree(pdfDoc, headings, bookmarkConfig) {
    ...
    const parentOf = filtered.map((_, i) => {
        for (let j = i - 1; j >= 0; j--) {
    ...
    for (let i = 0; i < filtered.length; i++) {
        ...
        pdfDoc.context.assign(itemRefs[i], pdfDoc.context.obj(entry)); // pdf-lib: obj() accepts Record<string,unknown> at runtime but types don't reflect it
    }
    ...
    pdfDoc.context.assign(outlineRef, pdfDoc.context.obj(rootEntry)); // pdf-lib: same runtime/type mismatch as above
    pdfDoc.catalog.set(PDFName.of('Outlines'), outlineRef);
    pdfDoc.catalog.set(PDFName.of('PageMode'), PDFName.of('UseOutlines'));
}
// ─── TOC Entry Rendering ────────────────────────────────────────────
export function renderTocEntry(pdfPage, pagedBlock, geo, fontMap) {
...
export function renderFormField(block, pdfPage, pdfDoc, fontMap, geo, yFromTop) {
...
export function renderSignaturePlaceholder(sig, pdfDoc, fontMap, geo) {
...
//# sourceMappingURL=render-extras.js.map
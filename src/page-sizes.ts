...
const PAGE_SIZES: Record<NamedPageSize, [number, number]> = {
...
export type PageSizeInput = NamedPageSize | [number, number] | undefined

/**
 * Resolve any page size input to [width, height] in points.
 * Default: A4 (595 × 842 pt)
 */
export function resolvePageDimensions(pageSize: PageSizeInput): [number, number] {
  if (Array.isArray(pageSize)) return [pageSize[0]!, pageSize[1]!]

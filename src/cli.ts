...
function parseArgs(argv: string[]): Args | { error: string } | { exit: 0 | 1; message: string } {
  const args: Args = { markdown: false }
  const positional: string[] = []

  for (let i = 0; i < argv.length; i++) {
    const a = argv[i]!
    if (a === '-h' || a === '--help') return { exit: 0, message: HELP }
    if (a === '-v' || a === '--version') return { exit: 0, message: pkg.version }
    if (a === '-i' || a === '--input') {
      const next = argv[++i]
      if (!next) return { error: `${a} requires a path argument` }
      args.inputPath = next
      continue
    }
    if (a === '-o' || a === '--output') {
    ...
    if (a === '--code-font') {
      const next = argv[++i]
      if (!next) return { error: `${a} requires a font family argument` }
      args.codeFont = next
      continue
    }
  ...
  if (!args.inputPath && positional[0]) args.inputPath = positional[0]
  if (!args.outputPath && positional[1]) args.outputPath = positional[1]
  if (positional.length > 2) return { error: `too many positional arguments (got ${positional.length}, max 2)` }
...
async function writeOutput(outputPath: string | undefined, bytes: Uint8Array): Promise<void> {
  if (outputPath) {
...
async function main(): Promise<number> {
  ...
  let doc: import('./types.js').PdfDocument
  if (parsed.markdown) {
    let markdownToContent: typeof import('./markdown.js')['markdownToContent']
    try {
    ...
    const opts: import('./markdown.js').MarkdownOptions = {}
    if (parsed.codeFont) opts.codeFontFamily = parsed.codeFont
    try {
    ...
    try {
      doc = JSON.parse(raw) as import('./types.js').PdfDocument
    } catch (err) {
  ...
  try {
    ...
    const code = (err as { code?: string } | null)?.code ?? 'RENDER_ERROR'

import type { ContentElement, PdfDocument, TableRow } from '../types.js'
...
function baseDoc(content: ContentElement[], overrides: Partial<PdfDocument> = {}): PdfDocument {
  return {
    pageSize: 'A4',
...
function richTextMixedSpans(): PdfDocument {
  return baseDoc([
...
function tableStress(): PdfDocument {
  const rows: TableRow[] = [
    {
      isHeader: true,
      cells: [
        { text: 'Item', fontWeight: 700 as const },
        { text: 'Category', fontWeight: 700 as const },
        { text: 'Qty', fontWeight: 700 as const, align: 'right' },
        { text: 'Amount', fontWeight: 700 as const, align: 'right' },
      ],
    },
    ...Array.from({ length: 14 }, (_, i) => ({
      cells: [
        ...
        { text: `₹${(i + 1) * 1250}`, align: 'right' as const },
      ],
    })) satisfies TableRow[],
  ]
...
function cjkLayout(): PdfDocument {
  return baseDoc([
...
function rtlLayout(): PdfDocument {
  return baseDoc([
...
function punctuationHeavy(): PdfDocument {
  return baseDoc([
...
function invoiceShowcase(): PdfDocument {
  return baseDoc([
      ...
      rows: [
        {
          isHeader: true,
          cells: [
            { text: 'Description', fontWeight: 700 as const },
            { text: 'Qty', fontWeight: 700 as const },
            { text: 'Rate', fontWeight: 700 as const },
            { text: 'Amount', fontWeight: 700 as const },
          ],
        },
...
function reportShowcase(): PdfDocument {
  return baseDoc([
...
export function getBenchmarkCorpora(): BenchmarkCorpus[] {

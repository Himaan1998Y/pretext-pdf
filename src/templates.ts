import type { ContentElement, TableCell, TableRow } from './types.js'
...
function cell(text: string, opts: Omit<TableCell, 'text'> = {}): TableCell {
  return { text, ...opts }
}
...
export function createInvoice(data: InvoiceData): ContentElement[] {
  const currency = data.currency ?? '$'
  const fmt = (n: number): string => `${currency}${n.toFixed(2)}`
  ...
  // Line items
  const itemRows: TableRow[] = data.items.map(item => {
    const lineTotal = item.total ?? item.quantity * item.unitPrice
    return row(
      cell(item.description),
      cell(String(item.quantity), { align: 'right' }),
      cell(fmt(item.unitPrice), { align: 'right' }),
      cell(fmt(lineTotal), { align: 'right' }),
    )
  })
  ...
  if (data.qrData) {
    ...
    elements.push({ type: 'qr-code', data: data.qrData, size: 80, align: 'left', spaceAfter: 12 } as ContentElement)
  }
...
export function createGstInvoice(data: GstInvoiceData): ContentElement[] {
  ...
  elements.push({ type: 'hr', spaceAfter: 8 } as ContentElement)
  ...
  elements.push({ type: 'paragraph', text: 'Bill To:', fontWeight: 700 } as ContentElement)
  ...
  const itemRows: TableRow[] = data.items.map((item, idx) => {
    ...
    return row(
      ...
      cell(String(item.quantity), { align: 'right' }),
      cell(item.unit),
      cell(formatINR(item.rate), { align: 'right' }),
      cell(formatINR(taxable), { align: 'right' }),
      cell(`${item.taxRate}%`, { align: 'right' }),
      cell(formatINR(tax), { align: 'right' }),
      cell(formatINR(lineTotal), { align: 'right' }),
    )
  })
  ...
  elements.push({
    ...
    rows: [
      row(
        cell('#', { fontWeight: 700 }), cell('Description', { fontWeight: 700 }), cell('HSN/SAC', { fontWeight: 700 }),
        cell('Qty', { fontWeight: 700, align: 'right' }), cell('Unit', { fontWeight: 700 }),
        cell('Rate', { fontWeight: 700, align: 'right' }), cell('Taxable', { fontWeight: 700, align: 'right' }),
        cell(taxColHeader, { fontWeight: 700, align: 'right' }), cell(taxAmtHeader, { fontWeight: 700, align: 'right' }),
        cell('Total', { fontWeight: 700, align: 'right' }),
      ),
      ...itemRows,
      row(
        cell('Total Taxable', { colspan: 6, align: 'right', fontWeight: 700 }),
        cell(formatINR(totalTaxable), { align: 'right', fontWeight: 700 }),
        cell(''), cell(''), cell(''),
      ),
      row(
        cell(`Total ${interState ? 'IGST' : 'CGST+SGST'}`, { colspan: 8, align: 'right' }),
        cell(formatINR(totalTax), { align: 'right', colspan: 2 }),
      ),
      row(
        cell('Grand Total', { colspan: 9, align: 'right', fontWeight: 700 }),
        cell(formatINR(grandTotal), { align: 'right', fontWeight: 700 }),
      ),
    ],
    spaceAfter: 12,
  })
  ...
  if (data.bankName || data.accountNumber) {
    elements.push({ type: 'paragraph', text: 'Bank Details:', fontWeight: 700 } as ContentElement)
    const bankLines = [
  ...
  if (data.qrUpiData) {
    ...
    elements.push({ type: 'qr-code', data: data.qrUpiData, size: 80, align: 'left', spaceAfter: 12 } as ContentElement)
  }
...
export function createReport(data: ReportData): ContentElement[] {
  const elements: ContentElement[] = []
  ...
  if (data.includeTableOfContents) {
    elements.push({ type: 'toc' } as ContentElement)
    elements.push({ type: 'page-break' } as ContentElement)
  }

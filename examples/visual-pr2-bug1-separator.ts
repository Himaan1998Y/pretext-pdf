 ...
 * 8-space run silently dropped, producing `·Antigravity Systems` flush
 * against the left margin instead of the intended deep indent.
 *
 * This example deliberately uses 8-space indents on continuation lines so
 * the visual delta between pre-fix and post-fix is obvious at a glance
 * (pre-fix: dots flush left; post-fix: dots at ~25pt indent).
 *
 * A vertical guide marker is injected at content-x=0 (where pre-fix
 * would have placed the dropped-indent dots) via a rendered pipe glyph
 * in the intro text so visual comparison is unambiguous.
 */
